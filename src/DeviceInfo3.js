...
const DeviceInfo3 = () => {
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      const result3 = await axios.get(`/api4/api/v1/devices`, {
        headers: headers3,
      });
      console.log(result3);
    ...
    <div className='parent-mid'><div className='mid container-mid'>
      {isLoading ? (
        ...
        deviceDetail3.map((el, index) => (
          <SingleDevice className='device-card' key={index} deviceSerial={el} endpoint="api4"/>
        ))
      )}
    <div className='nav-container'><a target="_blank" href="api4"><img src={arrow} className="arrow" /></a></div>
    </div>
    </div>
  );
};

export default DeviceInfo3;