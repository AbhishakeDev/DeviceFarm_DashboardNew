...
const DeviceInfo5 = () => {
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      const result3 = await axios.get(`/api6/api/v1/devices`, {
        headers: headers3,
      });
      console.log(result3);
    ...
    <div className='parent-mid'><div className='mid container-mid'>
      {isLoading ? (
        ...
        deviceDetail3.map((el, index) => (
          <SingleDevice className='device-card' key={index} deviceSerial={el} endpoint="api6"/>
        ))
      )}
    <div className='nav-container'><a target="_blank" href="api6"><img src={arrow} className="arrow" /></a></div>
    </div>
    </div>
  );
};
